...
than a command someone remembers to start. `toolbox/systemd/` carries a unit
template for that, along with an example configuration file and instructions:

```bash
cp toolbox/systemd/homi.conf.example /etc/xnvme/homi-1.conf
cp toolbox/systemd/xnvme-homi@.service /etc/systemd/system/
