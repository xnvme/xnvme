...
from the host: each connected process gets queues of its own to the same
namespace, and the controller is never asked which processes exist.
...
runs, but opening a device with a non-zero `cplane_id` fails with `-ENOTSUP`.
...
`--cplane-id` is required and sets the identifier the devices are opened
...
`cplane_id` first becomes the server (see
...
```bash
homi start 0000:03:00.0 --be upcie --cplane-id 1 --host_heap_size 134217728
```
...
Example: hold a single device open as the server for `cplane_id` 1:

```bash
homi start 0000:03:00.0 --be upcie --cplane-id 1
```

Example: hold several devices under the same `cplane_id`:

```bash
homi start 0000:03:00.0 0000:04:00.0 --be upcie --cplane-id 1
```

While it runs, other processes connect as clients by passing the same
`--cplane-id`:
...
three. A process reaching several controllers opens one connection per
controller, so it counts more than once. The per-controller `connections` is
today the same runtime-wide number rather than that controller's own.
...
controllers under its own `cplane_id`. Clients reach the one they want by
passing the matching id:

```bash
homi start 0000:03:00.0 0000:04:00.0 --be upcie --cplane-id 1
homi start 0000:05:00.0 0000:06:00.0 --be upcie --cplane-id 2
```
...
The unit is templated on `cplane_id`, matching the model above, so several
...
```bash
homi start 0000:03:00.0 --be upcie-cuda --cplane-id 1
```
