...
perf.homi: the homi that serves in the served mode (default: homi from PATH);
           point it into the same build tree as perf.xnvmeperf, since a
           client and a server from different builds do not belong together
...
def mode_enter(mode, bdfs, cijoe):
    ...
    if mode != "served":
        return 0
    homi = cijoe.getconf("perf.homi", "homi")
