...
def test_client_refuses_without_server(cijoe, device, be_opts, cli_args):
    """An id no server claimed makes the open fail rather than run direct"""

    if be_opts["be"] not in ("upcie", "upcie-cuda", "upcie-hip"):
        pytest.skip(reason="Only the uPCIe backends connect to a server by id")

    # Well above what the suite hands out, so it cannot collide with a server
    # the suite started for the other tests
    err, state = cijoe.run(
        f"xnvme info {device['uri']} --be {be_opts['be']} --homi-id 4242"
    )

    assert err, "the open succeeded with nobody serving the id"
    assert "xnvme_dev_open()" in state.output()
