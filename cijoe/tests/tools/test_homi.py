...
def _status(cijoe, cplane_id):
    """Run 'homi status' and return its exit code and parsed document"""

    err, state = cijoe.run(f"homi status --cplane-id {cplane_id}")
    output = state.output()
    lines = output.split("\n")
    ...
    start = next((i for i, ln in enumerate(lines) if ln.startswith("cplane_id:")), None)
...
def test_upcie_status_refuses_when_absent(cijoe):
    ...
    err, state = cijoe.run("homi status --cplane-id 4242")
    ...
    assert "cplane_id:" not in state.output()
...
def test_upcie_status_without_server(cijoe):
    ...
    err, doc = _status(cijoe, cplane_id)

    assert err, "status exits zero with no server running"
    assert doc["cplane_id"] == cplane_id
...
def test_status_with_server(cijoe, device, be_opts, cli_args):
    ...
    if not cplane_id:
        pytest.skip(reason="Requires a control-plane server; pass --cplane-id")
...
def test_queue_count_tracks_a_client(cijoe, device, be_opts, cli_args):
    ...
    cijoe.run(
        f"setsid xnvmeperf run {uri} --be {be_opts['be']} --cplane-id {cplane_id}"
...
def test_admin_costs_no_ioqueue(cijoe, device, be_opts, cli_args):
    ...
    cijoe.run(
        ...
        f" xnvme info {uri} --be {be_opts['be']} --cplane-id {cplane_id} >/dev/null 2>&1;"
...
def test_status_does_not_disturb_election(cijoe, device, be_opts, cli_args):
    ...
    err, _ = cijoe.run(
        ...
        f" --cplane-id {cplane_id} >/dev/null || exit 1; done'"
    )
...
def test_runtime_is_reachable_by_its_name(cijoe, device, be_opts, cli_args):
    ...
    That name is the whole rendezvous: a client derives it from --cplane-id and
    ...
    err, _ = cijoe.run(f"test -S /tmp/xnvme-homi-{cplane_id}.sock")
...
def _wait_for(cijoe, cplane_id, predicate, timeout):
    """Poll the server's account of itself until it says what is wanted"""

    connections, used = _counts(cijoe, cplane_id)
...
def _stop_clients(cijoe, uris):
    ...
    for uri in uris:
        cijoe.run(f"pkill -f 'xnvmeperf run {uri} ' || true")


def _start_client(cijoe, uri, be, cplane_id):
    """
...
def test_io_lands_on_the_controller_it_was_asked_for(cijoe, device, be_opts, cli_args):
    ...
    cijoe.run(
        ...
        f" --cplane-id {cplane_id} --iopattern randread --iosize 4096 --qdepth 8"
    ...
    connections, used = _wait_until(
        cijoe, cplane_id, lambda u: any(u.get(uri, 0) > 0 for uri in pair), 30
    )
    if not connections:
    ...
    ok, used = _wait_until(
        cijoe, cplane_id, lambda u: all(u.get(uri, 0) > 0 for uri in pair), 20
    )
...
def test_clients_come_and_go_across_controllers(cijoe, device, be_opts, cli_args):
    ...
    # Start from a server nobody else is talking to. A client an earlier case
    # left running would be counted into the baseline and then leave during the
    # wait below, which reads exactly like the new client never arriving. With
    # no clients the only connection is the one asking, so the count is one.
    _stop_clients(cijoe, uris)
    alone, connections, _ = _wait_for(cijoe, cplane_id, lambda c, _u: c == 1, 60)
    assert alone, (
    ...
    try:
        for shape in shapes:
            ...
            for uri in shape:
                _start_client(cijoe, uri, be, cplane_id)
...
def test_a_queue_survives_a_release_on_another_controller(
    ...
    base = _used(cijoe, cplane_id)

    cijoe.run(
        f"setsid sh -c 'xnvmeperf run {holder['uri']} --be {be} --cplane-id {cplane_id}"
        f" --iopattern randread --iosize 4096 --qdepth 8 --nqueues 1"
...
def test_a_client_outlives_the_server_shutting_down(cijoe, device, be_opts, cli_args):
    ...
    cijoe.run(
        f"setsid sh -c 'xnvmeperf run {uri} --be {be} --cplane-id {cplane_id}"
        f" --iopattern randread --iosize 4096 --qdepth 8 --nqueues 1"
        f" --runtime {runtime} --cpulist 0; echo $? > {rc_path}'"
    ...
    took, used = _wait_until(
        cijoe, cplane_id, lambda u: u.get(uri, 0) > base.get(uri, 0), 30
    )
    assert took, (
...
def test_the_two_spellings_name_one_identifier(cijoe, device, be_opts, cli_args):
    """
    A client passing --shm_id reaches a server started with --cplane-id.
    ...
    them is what says so: the server here was started by the fixture with
    --cplane-id, and this attaches to it by the name that came before.
    ...
    err, state = cijoe.run(f"homi status --shm_id {cplane_id}")
    assert not err, "status did not answer for the identifier given as --shm_id"
    assert f"cplane_id: {cplane_id}" in state.output(), (
        "status answered for a different identifier than the one asked for;"
        f" output was {state.output()!r}"
    )


def _stall_a_connection(cijoe, cplane_id, uri, seconds):
    """
    Connect to a controller's socket, send part of a message, and hold it open.
    ...
    sane = re.sub(r"[:./]", "-", uri)
    sock = f"/tmp/xnvme-homi-{cplane_id}-{sane}.sock"
...
def test_a_stalled_client_holds_up_nobody(cijoe, device, be_opts, cli_args):
    ...
    err, _ = _stall_a_connection(cijoe, cplane_id, uri, 20)
...
def _speak(cijoe, cplane_id, uri, op, version=_MSG_VERSION, payload_at=None):
    """
    Send one message to a controller's socket and return the reply's status.
    ...
    poke = (
        ...
        if payload_at is None
            ...
            f"m[{payload_at[0]}:{payload_at[0]} + 4] ="
            f" __import__('struct').pack('<I', {payload_at[1]});"
        )
    )
    ...
    err, state = cijoe.run(
        ...
        f"m[0:16]=struct.pack('<IIiI', {op}, {version}, 0, 0);"
        f"{poke}"
        f"s=socket.socket(socket.AF_UNIX,socket.SOCK_STREAM);s.connect('{sock}');"
        "s.sendall(bytes(m));"
        f"r=b''" + ";"
...
def test_a_stranger_version_is_refused(cijoe, device, be_opts, cli_args):
    ...
    status = _speak(cijoe, cplane_id, device["uri"], _OP_STATUS, version=0xBADF00D)
...
def test_an_unknown_request_is_refused(cijoe, device, be_opts, cli_args):
    ...
    status = _speak(cijoe, cplane_id, device["uri"], 99)
...
def test_asking_for_resources_before_initialising_is_refused(
    ...
    # Queue depth at offset 48, and a byte count at 16, so each request is one
    # the server would otherwise honour.
    status = _speak(cijoe, cplane_id, uri, _OP_ALLOC_IOQPAIR, payload_at=(48, 8))
    assert status == -107, f"expected -ENOTCONN for a queue before init, got {status}"

    status = _speak(cijoe, cplane_id, uri, _OP_ALLOC_BUF, payload_at=(16, 4096))
...
def test_handing_back_what_was_never_held_is_refused(cijoe, device, be_opts, cli_args):
    ...
    status = _speak(cijoe, cplane_id, uri, _OP_FREE_IOQPAIR, payload_at=(16, 4095))
    assert status == -2, f"expected -ENOENT for a queue never held, got {status}"

    status = _speak(cijoe, cplane_id, uri, _OP_FREE_BUF, payload_at=(16, 0xDEAD))
...
def test_a_stalled_client_does_not_stop_a_working_one(cijoe, device, be_opts, cli_args):
    ...
    err, _ = _stall_a_connection(cijoe, cplane_id, uri, stall)
    assert not err, "could not open a connection to stall"
    ...
    err, _ = cijoe.run(
        ...
        f" --sync {be_opts['be']} --cplane-id {cplane_id}"
    )
    elapsed = time() - started
