...
"""
    ...
    This has only been tested using meson 0.61, requires the following files
    from the builddir:
    ...
    NOTE: This is implemented as a quick'n'dirty replacement for the
    CMake/CPack feature.
"""
import subprocess
import argparse
...
def emit_control_file(args):
    ...
    with open(os.path.join(
        args.builddir, 'meson-info', 'intro-projectinfo.json'
    )) as pfd:
        pinfo = json.load(pfd)

    deb_info = DEFAULT
    deb_info["deb_version"] = pinfo.get("version", DEFAULT["deb_version"])
    deb_info["deb_package"] = pinfo.get(
        "descriptive_name",
        DEFAULT["deb_package"]
    )
...
def main(args):
    ...
    with open(os.path.join(
        args.builddir, 'meson-info', 'intro-installed.json'
    )) as ifd:
