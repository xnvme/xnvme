...
int
xnvme_be_spdk_ctrlr_get_registers(const struct xnvme_dev *dev, struct xnvme_spec_ctrlr_bar *bar)
{
	struct xnvme_be_spdk_state *state = (void *)dev->be.state;
	struct spdk_nvme_registers *regs =
		(struct spdk_nvme_registers *)spdk_nvme_ctrlr_get_registers(state->ctrlr);
