...
/** What per-I/O state written by one thread is kept apart from another's by */
#define XNVME_BE_UPCIE_CACHE_LINE 64
				  ...
				  uint16_t depth)
{
	...
	/* The pool's top is written on every submission and completion, so the
	 * pool gets cache lines of its own. A plain calloc() ends it in the line
	 * the next allocation begins in, and when a process opens its queues
	 * back to back that is the next queue's header, which another thread
	 * then writes on every I/O of its own: the two bounce the line between
	 * their cores, and each loses a quarter of its throughput to it. */
	nbytes = (sizeof(*qpair->rpool) + XNVME_BE_UPCIE_CACHE_LINE - 1) &
		 ~(size_t)(XNVME_BE_UPCIE_CACHE_LINE - 1);
	err = posix_memalign(&rpool, XNVME_BE_UPCIE_CACHE_LINE, nbytes);
	if (err) {
		return -err;
	}
	memset(rpool, 0, nbytes);
	qpair->rpool = rpool;
