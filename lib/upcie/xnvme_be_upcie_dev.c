 ...
 * initialized RTE matches. A non-zero opts->cplane_id names a runtime another
...
_rte_init(enum xnvme_be_upcie_mode mode, struct xnvme_opts *opts, const char *bdf)
{
	...
	if (opts->cplane_id) {
		...
		err = xnvme_be_upcie_cplane_init_connection(opts->cplane_id, bdf, NULL);
		if (!err) {
...
xnvme_be_upcie_ctrlr_init(struct xnvme_dev *dev)
{
	...
	ctrlr->sock = -1;
	pthread_mutex_init(&ctrlr->sock_lock, NULL);
	...
	if (g_upcie_rte.connection.alive) {
		...
		err = xnvme_be_upcie_cplane_init_connection(dev->opts.cplane_id, dev->ident.uri,
							    ctrlr);
		if (err) {
...
xnvme_be_upcie_ctrlr_term(void *handle)
{
	...
	if (g_upcie_rte.connection.alive) {
		...
		pthread_mutex_destroy(&ctrlr->sock_lock);
		free(ctrlr->ctrl);
		free(ctrlr);
