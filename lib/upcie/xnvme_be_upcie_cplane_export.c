...
_region_iova_alloc(uint64_t nbytes, uint64_t align)
{
	...
	/* Two lists share the window: the regions installed here for clients,
	 * and what the handle installed for this process's own heap, which a
	 * server serving from device memory has there before any client asks.
	 * A candidate has to clear both. */
	while (moved) {
		moved = 0;
		...
		for (const struct dmamem_iommu_map_pa_mapping *m = imp->maps; m; m = m->next) {
