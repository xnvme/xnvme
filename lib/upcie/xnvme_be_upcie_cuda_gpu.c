...
xnvme_cuda_queue_create(struct xnvme_dev *dev, uint16_t depth, struct xnvme_cuda_queue **queue)
{
	 ...
	 * asked for. */
	{
		...
		size_t sq_nbytes = (size_t)(depth + 1) * 64;
		size_t cq_nbytes = (size_t)(depth + 1) * 16;
		uint64_t heap_base = g_upcie_cuda_rte.cuda_heap.vaddr;
		int clock_rate_khz = 0;
		uint32_t qid = 0;
		CUdevice cu_dev;

		if (!g_upcie_cuda_rte.db_base) {
		...
		if (!_qpair.sq || !_qpair.cq) {
			...
			cudamem_heap_block_free(&g_upcie_cuda_rte.cuda_heap, _qpair.sq);
			cudamem_heap_block_free(&g_upcie_cuda_rte.cuda_heap, _qpair.cq);
			cuMemFree((CUdeviceptr)qpair);
			return -ENOMEM;
		}
		...
		if (g_upcie_rte.connection.alive) {
			if (!g_upcie_cuda_rte.reg_offset) {
				...
				err = xnvme_be_upcie_cplane_alloc_qpair_at(
					state->ctrlr, g_upcie_cuda_rte.reg_offset,
		...
		{
			uint8_t *db = g_upcie_cuda_rte.db_base;
			int dstrd = nvme_reg_cap_get_dstrd(nvme_mmio_cap_read(db));

			_qpair.sqdb = db + 0x1000 + ((2 * qid) << (2 + dstrd));
			_qpair.cqdb = db + 0x1000 + ((2 * qid + 1) << (2 + dstrd));
		}
