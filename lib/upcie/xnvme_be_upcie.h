...
struct xnvme_be_upcie_ctrlr {
	 ...
	 * describe them all from whichever it opened first. Operations on
	 * separate queues need no synchronisation from the caller, and two of
	 * them meet here, so a request and its reply are one turn under this
	 * lock rather than two threads interleaving on the one stream. */
	int sock;                                 ///< To the server; -1 when not connected
	pthread_mutex_t sock_lock;                ///< Held across a request and its reply
 ...
 * sets `u.status.index` before each call. It comes back as the reply.
 */
int
