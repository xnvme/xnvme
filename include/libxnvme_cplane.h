...
struct xnvme_cplane_info {
	 ...
	 *
	 * Connections rather than processes: a process reaching several
	 * controllers opens one per controller, so it counts more than once.
	 */
 ...
 * when it disconnects.
 *
 * Threads are used internally, one per connection, so that a client waiting on
 * something slow does not hold up the rest. They are created and joined here;
 * a caller sees only the blocking call.
 *
