...
#define XNVME_IDENT_URI_LEN 320

/**
 * @enum xnvme_ident_type
 */
enum xnvme_ident_type {
	XNVME_IDENT_CTRLR = 0x1, ///< XNVME_IDENT_CTRLR
	XNVME_IDENT_NS = 0x2, ///< XNVME_IDENT_NS
};

/**
 * Produces a string representation of the given ::xnvme_ident_type
 *
 * @param idt the enum value to produce a string representation of
 *
 * @return On success, a string representation is returned. On error, the string
 * "XNVME_IDENT_ENOSYS" is returned.
 */
const char *
xnvme_ident_type_str(enum xnvme_ident_type idt);

/**
 * Representation of devices identifiers once decoded from text-representation
 *
 * @struct xnvme_ident
 */
struct xnvme_ident {
	uint32_t nsid;			///< NVMe Namespace Identifier
	uint16_t bid;			///< Internal backend-identifier
	uint8_t type;			///< Identifier type
	uint8_t _pad0;
	enum xnvme_spec_nstype nst;	///< nst; only valid when nsid != 0
	uint8_t _pad1[52];
	char be_uri[XNVME_IDENT_URI_LEN];	///< uri used by backend
	char uri[XNVME_IDENT_URI_LEN];		///< uri provided by user
};
 ...
 *
 * @return On success, 0 is returned. On error, -1 is return and `errno` set to
 * indicate the error.
 */
int
...
/**
 * List of devices found on the system usable with libxnvme
 *
...
/**
 * Print the given list of enumerated devices
 *
 * @param list The device list to print
 * @param opts Printer options
 */
 ...
 *
 * @note the caller must free the returned structure
 *
 * @param opts System enumeration options
 *
 * @return On success, a list populated with usable device identifiers is
...
/**
 * Opaque asynchronous context as returned by xnvme_async_init()
 *
 ...
 *
 * TODO: Consider how to align this with the NVMe completion-entry and do proper
 * error-handling for SPDK as well as IOCTLs
 *
 * @struct xnvme_req
 */
struct xnvme_req {
	...
	struct xnvme_async_cmd_ctx async;	///< ASYNC command context
};
...
struct xnvme_geo {
	 ...
	 */
	size_t npugrp;		///< Nr. of Parallel Unit Groups
	size_t npunit;		///< Nr. of Parallel Units in PUG

	size_t nzone;		///< Nr. of zones in PU
	size_t nsect;		///< Nr. of sectors per zone
	size_t nbytes;		///< Nr. of bytes per sector
	size_t nbytes_oob;	///< Nr. of bytes per sector in OOB

	size_t tbytes;		///< Total # bytes in geometry

	size_t mdts_nbytes;	///< Maximum data transfer size in bytes
};
 ...
 * indicate and req filled with lower-level status codes
 */
int
