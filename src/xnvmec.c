...
void
xnvmec_buf_fill(void *buf, size_t nbytes)
{
	uint8_t *cbuf = buf;
...
void
xnvmec_pinfo(const char *format, ...)
{
	va_list args;
...
void
xnvmec_perror(const char *msg)
{
	const int max = 210;
	char buf[max];

	snprintf(buf, max - 10, "# perror: '%s'", msg);
	perror(buf);
...
xnvmec_parse(struct xnvmec *cli)
{
	...
	if (cli->argc < 2) {
		xnvmec_pinfo("Insufficient arguments: no <command> given");
		errno = EINVAL;
		return -1;
	}
	...
	if (!sub) {
		xnvmec_pinfo("%s: invalid command: '%s'", cli->argv[0],
			     sub_name);
		errno = EINVAL;
		return -1;
	}
	...
	for (int oi = 0; oi < XNVMEC_SUB_OPTS_LEN; ++oi) {
		...
		attr = xnvmec_opt_attr_by_opt(opt->opt, xnvmec_opts);
		if (!attr) {
			xnvmec_pinfo("Invalid arguments: cannot parse value");
			errno = EINVAL;
			return -1;
		}
	...
	for (int count = 0; count < signature.total_long; ++count) {
		...
		if (xnvmec_assign_arg(cli, ret, optarg, opti->type)) {
			...
			xnvmec_pinfo("invalid argument value(%s)", optarg);
			errno = EINVAL;
			return -1;
		}
	}
	...
	if (signature.posa) {
		...
		if (cli->argc < 3) {
			xnvmec_pinfo("Insufficient arguments, see: --help");
			errno = EINVAL;
			return -1;
		}
		...
		for (int pos = 0; pos < signature.posa; ++pos) {
			...
			if (xnvmec_assign_arg(cli, attr->opt, cli->argv[idx],
					      XNVMEC_POSA)) {
				XNVME_DEBUG("FAILED: xnvmec_assign_arg()");
				xnvmec_pinfo("invalid argument value(%s)",
					     cli->argv[idx]);
				errno = EINVAL;
				return -1;
			}
	...
	if ((!args->help) && (parsed.total_req < signature.total_req)) {
		xnvmec_pinfo("Insufficient required arguments, see: --help");
		errno = EINVAL;
		return -1;
	}
