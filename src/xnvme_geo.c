	       ...
	       const char *sep, int head)
{
	...
	wrtn += fprintf(stream, "%*snpugrp: %u%s", indent, "",
			geo->npugrp, sep);
	wrtn += fprintf(stream, "%*snpunit: %u%s", indent, "",
			geo->npunit, sep);
	wrtn += fprintf(stream, "%*snzone: %u%s", indent, "",
			geo->nzone, sep);
	wrtn += fprintf(stream, "%*snsect: %zu%s", indent, "",
			geo->nsect, sep);
	wrtn += fprintf(stream, "%*snbytes: %u%s", indent, "",
			geo->nbytes, sep);
	wrtn += fprintf(stream, "%*snbytes_oob: %u%s", indent, "",
			geo->nbytes_oob, sep);
	...
	wrtn += fprintf(stream, "%*slba_nbytes: %u%s", indent, "",
			geo->lba_nbytes, sep);
	wrtn += fprintf(stream, "%*slba_extended: %u%s", indent, "",
			geo->lba_extended, sep);

	wrtn += fprintf(stream, "%*sssw: %"PRIu64, indent, "",
			geo->ssw);
