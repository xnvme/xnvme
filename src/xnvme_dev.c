...
xnvme_dev_openf(const char *dev_uri, int opts)
{
	...
	dev = xnvme_be_factory(dev_uri, opts);
	if (!dev) {
	...
	if (opts & XNVME_CMD_MASK_IOMD) {
		dev->cmd_opts |= opts & XNVME_CMD_MASK_IOMD;
	} else {
		dev->cmd_opts |= XNVME_CMD_DEF_IOMD;
	}

	if (opts & XNVME_CMD_MASK_UPLD) {
		dev->cmd_opts |= opts & XNVME_CMD_MASK_UPLD;
	} else {
