	       ...
	       struct xnvme_req *ret)
{
	...
	return dev->be->cmd_pass(dev, cmd, dbuf, dbuf_nbytes, mbuf,
		     ...
		     size_t mbuf_nbytes, int opts, struct xnvme_req *ret)
{
	...
	return dev->be->cmd_pass_admin(dev, cmd, dbuf, dbuf_nbytes, mbuf,
				       mbuf_nbytes, opts, ret);
}
	      ...
	      uint32_t dbuf_nbytes, struct xnvme_req *ret)
{
	...
	return dev->be->cmd_pass_admin(dev, &cmd, dbuf, dbuf_nbytes, NULL, 0x0,
				       0x0, ret);
}
...
xnvme_cmd_idfy(struct xnvme_dev *dev, uint8_t cns, uint16_t cntid, uint8_t nsid,
	       ...
	       struct xnvme_spec_idfy *dbuf, struct xnvme_req *ret)
{
...
xnvme_cmd_idfy_ctrlr(struct xnvme_dev *dev, struct xnvme_spec_idfy *dbuf,
		     struct xnvme_req *ret)
{
	return xnvme_cmd_idfy(dev, XNVME_SPEC_IDFY_CTRLR, 0x0, 0x0, 0x0, 0x0,
			      0x0, dbuf, ret);
}
...
xnvme_cmd_idfy_ns(struct xnvme_dev *dev, uint32_t nsid, uint8_t nstype,
		  struct xnvme_spec_idfy *dbuf, struct xnvme_req *ret)
{
	return xnvme_cmd_idfy(dev, XNVME_SPEC_IDFY_NS, 0x0,
			      (nsid) ? nsid : xnvme_dev_get_nsid(dev), nstype,
			      0, 0x0, dbuf, ret);
}
		...
		size_t dbuf_nbytes, struct xnvme_req *ret)
{
	...
	return dev->be->cmd_pass_admin(dev, &cmd, (void *)dbuf, dbuf_nbytes,
				       NULL, 0x0, 0x0, ret);
}
		 ...
		 struct xnvme_req *ret)
{
	...
	return dev->be->cmd_pass_admin(dev, &cmd, NULL, 0, NULL, 0, 0x0, ret);
}
		...
		struct xnvme_req *ret)
{
	...
	return dev->be->cmd_pass(dev, &cmd, cdbuf, dbuf_nbytes, cmbuf,
				 mbuf_nbytes, opts, ret);
}
